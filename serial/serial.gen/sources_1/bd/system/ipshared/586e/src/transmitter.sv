module transmitter(
    ...
    begin
        ...
        if (baud_en && !baud_en_prev)      // if enable is set in the control register and if rising edge of baud rate clock
        begin
            case(state)
                IDLE:
                begin
                ...
                begin
                    ...
                    else if (size == 2'b01 || size == 2'b10 || size == 2'b11)
                    begin
                        state <= D5;
                    end
                end
                ...
                begin
                    ...
                    reg [3:0] ones;
                    ones <= 0;
                    ones <= latched_data[0] + latched_data[1] + latched_data[2] + latched_data[3] + latched_data[4] + latched_data[5] + latched_data[6] + latched_data[7] + latched_data[8];
                    // for (n = 0; n < (size + 5); n = n + 1)                // count the number of 1's in the data word
                    // begin
                    ...
                    state <= STOP1;
                    test[8:0] <= {out_data, latched_data[7:0]};
                end           
                STOP1:
                begin
                    out_data <= 1;
