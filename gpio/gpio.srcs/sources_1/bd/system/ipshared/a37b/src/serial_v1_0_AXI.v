...
`timescale 1 ns / 1 ps
    ...
    begin
        ...
        begin
            if (axi_arvalid && axi_arready && ~axi_rvalid && (rd_index == rd_index_old + 1))
            begin
                axi_rvalid <= 1'b1;
                axi_rresp <= 2'b0;
                rd_index_old <= rd_index;
            end
...
//     .pulse(clear_overflow_request));
  ...
  // Fifo
   ...
   .reset(axi_resetn),
   .wr_data(wr_data[8:0]),
   .wr_request(ok_to_write && fifo_wr_en),
   .rd_request(ok_to_read && fifo_rd_en),
   ...
   assign rd_data[8:0] = rd_data[8:0];
