...
// handle clear overflow signal
   always_ff @ (posedge(axi_clk))
   begin
        // assign clear_overflow_request signal based on status register
        clear_overflow_request <= status[0];
        // clear the status[0] bit so that it's active for only one clock when set
        if(status[0] == 1)
        begin
            status[0] <= 0;
        end
